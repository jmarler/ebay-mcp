...
A local [MCP](https://modelcontextprotocol.io) server exposing 301 tools across 100% of eBay's Sell APIs. TypeScript/Node.js (ESM), built with `@modelcontextprotocol/sdk`, Effect-backed validation, a Zod SDK adapter, and OpenAPI-generated types.
...
| `tools/`         | MCP tool wiring — `registry.ts`, `contracts.ts`, `schemas.ts`, `defineTool.ts`, `categories/`, and `media/` (handler-side image-upload input resolver that does MCP-boundary file/base64 I/O before the Media API client sees bytes) (family files co-locate each tool definition with its handler via `defineTool`; marketing pairs a large definition catalog with a handler map and must keep one public definition per handler) |
